 ...
 function init() {
  ...
  for (let row = 0; row < rows; row++)
   for (let column = 0; column < columns; column++) {
    ...
    tile[row][column].style = 'position:relative;height:30px; width: 30px; margin:5px;';
 ...
 function click(event) {
  ...
  if (event.which == 1 && picture[row][column] != 'flag') {
   if (board[row][column] == 'mine') {
    ...
    status.innerHTML = 'GAME OVER<br>Your score was: <p>' + points.toString() + ' <br>Click here to restart';
   } else
 ...
 $(document).ready(function() {
  $('#button').click(function() {
   ...
   $.ajax({
    url: '/scores',
    type: 'PUT',
    data: { full_name: name, scores: points },
    success: function(response) {
     console.log(response);
     console.log(response.full_name);
     var name = "<p>Name: " + response.full_name;
     var highscore = "<br>Highscore: " + response.scores + "</p>";
